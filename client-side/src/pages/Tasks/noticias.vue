      ...
      <!-- listgrid -->
       <div class="p-5">
            <div class="flex justify-between">
                <span>Ultimas noticias sobre {{ task.titulo }}</span>
                <button class="text-sm rounded-lg px-2 py-1" :class="[ task.status == 'Concluído' ? 'text-gray-400 bg-gray-100' : 'text-blue-400 bg-blue-100' ]" :disabled="task.status == 'Concluído'" @click="SearchNews(task._id)">Procurar Mais</button>
            </div>
            <div>
                <div class="grid grid-cols-4 py-2">
                    <li v-for="(new_inner,index) in localNews" :key="index" class="my-1 px-2">
                        <a :href="new_inner.link" class="underline text-blue-400">{{ new_inner.link.split("/").at(-1).split(".html").at(0) }}</a>
                    </li>
                </div>
            </div>
       </div>
