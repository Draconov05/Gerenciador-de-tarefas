    ...
    <div class="border rounded-md h-[15vh] w-[20vw] px-2" @mouseover="show_menu = true" @mouseleave="show_menu = false" >
        <div class="h-1/3 flex items-center justify-between">
            ...
            <div class="flex space-x-2 relative">
                <BadgeStatus :value="task.status" :tipo="task.status"></BadgeStatus>
                <div v-if="show_menu" class="flex absolute z-10 right-1 bg-white p-2 border rounded-md space-x-2">
        ...
        <div class="h-1/3 overflow-hidden flex">
            <a class="text-blue-300 underline " :href="task.link ? task.link[0].link : task.link">link</a>
        </div>
...
export default {
    ...
    methods:{
        OpenModalEdit(){
            this.$emit("modal_edit",this.task._id)
        },
        OpenDelete(){
